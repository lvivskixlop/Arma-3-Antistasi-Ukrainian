...
if (str (_display) != "no display") then
{
	...
	_ChildControl  ctrlSetTooltip format ["Ціна: %1 ₴",server getVariable (SDKEng select 0)];
	_ChildControl = _display displayCtrl 108;
	_ChildControl  ctrlSetTooltip format ["Ціна: %1 ₴",server getVariable (SDKExp select 0)];
	_ChildControl = _display displayCtrl 109;
	_ChildControl  ctrlSetTooltip format ["Ціна: %1 ₴",server getVariable (SDKGL select 0)];
	if (hasIFA) then {_childControl ctrlSetText "Радист"};
