...
//if (_carryX getVariable ["carryX",false]) exitWith {hint "Цього бійця зара несуть, і ви не можете йому отак допомогти."};
if (!alive _carryX) exitWith {hint format ["%1 is dead",name _carryX]};
if !(_carryX getVariable ["INCAPACITATED",false]) exitWith {hint format ["%1 більше не потребує допомоги.",name _carryX]};
if !(isNull attachedTo _carryX) exitWith {hint format ["%1 вже переноситься, тому ви не можете його переносити.",name _carryX]};
...
_carryX setDir 180;
_timeOut = time + 60;
_action = _carrierX addAction [format ["Відпустити %1",name _carryX], {{detach _x} forEach (attachedObjects player)},nil,0,false,true,"","(isPlayer _this) and (_this == _this getVariable ['owner',objNull])"];
