...
if (_unit getVariable ["helping",false]) exitWith {if (_inPlayerGroup) then {_unit groupChat "Не можу переозброїтись вже. Лікую товариша."}};

private _rearming = _unit getVariable ["rearming",false];
if (_rearming) exitWith {if (_inPlayerGroup) then {_unit groupChat "Я вже переозброююсь. Відміна."; _unit setVariable ["rearming",false]}};
	...
	if ((_foundItem) && (_unit getVariable "rearming")) then {
		...
		if (_unit distance _selectedContainer < 3) then {
			...
			if (primaryWeapon _unit isEqualTo _selectedWeapon) then {
				if (_inPlayerGroup) then {_unit groupChat "В мене краща зброя."};//I have a better weapon now
...
if ((_foundItem) && (_unit getVariable "rearming")) then {
	...
	if (_unit distance _selectedContainer < 3) then {
		...
		if ({_x in _primaryMagazines} count (magazines _unit) >= _targetMagazines) then {
			if (_inPlayerGroup) then {_unit groupChat "Переозброєний"};
		} else {
			if (_inPlayerGroup) then {_unit groupChat "Частково переозброєний"};
		};
	};
	_selectedContainer setVariable ["busy",false];
} else {
	if (_inPlayerGroup) then {_unit groupChat "Нема де взяти основну зброю"};
};
...
if ((_secondaryWeapon == "") && (loadAbs _unit < 340)) then {
	...
	if ((_foundItem) && (_unit getVariable "rearming")) then {
		...
		if ((!alive _selectedContainer) || (!(_selectedContainer isKindOf "ReammoBox_F"))) then {_selectedContainer setVariable ["busy",true]};
		_unit doMove (getPosATL _selectedContainer);
		if (_inPlayerGroup) then {_unit groupChat "Підбираю пістоля"};
		_timeOut = time + 60;
		sleep 1;
		waitUntil {!([_unit] call A3A_fnc_canFight) || (isNull _selectedContainer) || (_unit distance _selectedContainer < 3) || (_timeOut < time) || (unitReady _unit)};
		if ((unitReady _unit) && ([_unit] call A3A_fnc_canFight) && (_unit distance _selectedContainer > 3) && (_selectedContainer isKindOf "ReammoBox_F") && (!isNull _selectedContainer)) then {_unit setPos position _selectedContainer};
		if (_unit distance _selectedContainer < 3) then {
			...
			if (secondaryWeapon _unit == _selectedWeapon) then {
				if (_inPlayerGroup) then {_unit groupChat "Тепер я маю пістоля"};
...
if (_secondaryWeapon != "") then {
	...
	if ((_foundItem) && (_unit getVariable "rearming")) then {
		...
		if (_inPlayerGroup) then {_unit groupChat "Переозброююсь"};
		_timeOut = time + 60;
		sleep 1;
		waitUntil {!([_unit] call A3A_fnc_canFight) || (isNull _selectedContainer) || (_unit distance _selectedContainer < 3) || (_timeOut < time) || (unitReady _unit)};
		if ((unitReady _unit) && ([_unit] call A3A_fnc_canFight) && (_unit distance _selectedContainer > 3) && (_selectedContainer isKindOf "ReammoBox_F") && (!isNull _selectedContainer)) then {_unit setPos position _selectedContainer};
		...
		if (_inPlayerGroup) then {_unit groupChat "Нема де взяти набої до пістоля"};
	};
	sleep 3;
};
...
if (!haveRadio && {_unit call A3A_fnc_getRadio == ""}) then {
	...
	if ((_foundItem) && (_unit getVariable "rearming")) then {
		...
		_unit doMove (getPosATL _selectedContainer);
		if (_inPlayerGroup) then {_unit groupChat "Підбираю рацію"};
		_timeOut = time + 60;
		sleep 1;
		waitUntil {!([_unit] call A3A_fnc_canFight) || (isNull _selectedContainer) || (_unit distance _selectedContainer < 3) || (_timeOut < time) || (unitReady _unit)};
		if (_unit distance _selectedContainer < 3) then {
...
if (hmd _unit == "") then {
	...
	if ((_foundItem) && (_unit getVariable "rearming")) then {
		...
		_unit doMove (getPosATL _selectedContainer);
		if (_inPlayerGroup) then {_unit groupChat "Підбираю ПНБ"};
		_timeOut = time + 60;
		sleep 1;
		waitUntil {!([_unit] call A3A_fnc_canFight) || (isNull _selectedContainer) || (_unit distance _selectedContainer < 3) || (_timeOut < time) || (unitReady _unit)};
		if (_unit distance _selectedContainer < 3) then {
...
if (!(headgear _unit in allArmoredHeadgear)) then {
	...
	if ((_foundItem) && (_unit getVariable "rearming")) then {
		...
		_unit doMove (getPosATL _selectedContainer);
		if (_inPlayerGroup) then {_unit groupChat "Беру шолома."};
		_timeOut = time + 60;
		sleep 1;
		waitUntil {!([_unit] call A3A_fnc_canFight) || (isNull _selectedContainer) || (_unit distance _selectedContainer < 3) || (_timeOut < time) || (unitReady _unit)};
		if (_unit distance _selectedContainer < 3) then {
...
if ({_x == "FirstAidKit"} count (items _unit) < _targetFAKs) then {
	...
	if ((_foundItem) && (_unit getVariable "rearming")) then {
		...
		_selectedContainer setVariable ["busy",true];
		_unit doMove (getPosATL _selectedContainer);
		if (_inPlayerGroup) then {_unit groupChat "Підбираю медикаменти"};
		_timeOut = time + 60;
		sleep 1;
		waitUntil {!([_unit] call A3A_fnc_canFight) || (isNull _selectedContainer) || (_unit distance _selectedContainer < 3) || (_timeOut < time) || (unitReady _unit)};
		if (_unit distance _selectedContainer < 3) then {
...
if ((_foundItem) && (_unit getVariable "rearming")) then {
	...
	_selectedContainer setVariable ["busy",true];
	_unit doMove (getPosATL _selectedContainer);
	if (_inPlayerGroup) then {_unit groupChat "Беру кращий бронік"};
	_timeOut = time + 60;
	sleep 1;
	waitUntil {!([_unit] call A3A_fnc_canFight) || (isNull _selectedContainer) || (_unit distance _selectedContainer < 3) || (_timeOut < time) || (unitReady _unit)};
	if (_unit distance _selectedContainer < 3) then {
...
if (backpack _unit == "") then {
	...
	if ((_foundItem) && (_unit getVariable "rearming")) then {
		...
		_selectedContainer setVariable ["busy",true];
		_unit doMove (getPosATL _selectedContainer);
		if (_inPlayerGroup) then {_unit groupChat "Беру рюкзака"};
		_timeOut = time + 60;
		sleep 1;
		waitUntil {!([_unit] call A3A_fnc_canFight) || (isNull _selectedContainer) || (_unit distance _selectedContainer < 3) || (_timeOut < time) || (unitReady _unit)};
		if (_unit distance _selectedContainer < 3) then {
...
if (!_needsRearm) then {if (_inPlayerGroup) then {_unit groupChat "Не потрібно переозброюватись"}} else {if (_inPlayerGroup) then {_unit groupChat "Переозброївся"}};
