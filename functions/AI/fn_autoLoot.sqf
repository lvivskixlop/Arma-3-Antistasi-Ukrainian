...
if (_unit getVariable ["helping",false]) exitWith {_unit groupChat "Та не можу я зара патрони шукати. Я товариша лікую."};
_rearming = _unit getVariable "rearming";
if (_rearming) exitWith {_unit groupChat "Я переозброююсь. Відміна."; _unit setVariable ["rearming",false]};
if (_unit == gunner _truckX) exitWith {_unit groupChat "Я не можу поповнити боєзапас. Я сижу за цією зброєю."};
if (!canMove _truckX) exitWith {_unit groupChat "Не бачу багато сенсу в переозброєнні моєї техніки, бо вона поламана і потребує ремонту."};
...
if (count _objectsX == 0) exitWith {_unit groupChat "Не бачу трупаків, щоб полутати."};
...
if (isNull _target) exitWith {_unit groupChat "Тут нема нічого."};
_target setVariable ["busy",true];
_unit setVariable ["rearming",true];
_unit groupChat "Починаю лутатись.";
...
if (!_continuar) then
	{
	_unit groupChat "Вже нема ніякої зброї"
	};
