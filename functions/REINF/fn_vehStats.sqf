if (count hcSelected player == 0) exitWith {hint "Треба вибрати якусь групу в меню вищого командування."};
...
if (_esStatic) exitWith {hint "Не можна керувати відділеннями зі станковою зброєю."};
*/
...
player sideChat format ["%1, ВАШ СТАТУС!!",groupID _groupX];
_unitsX = units _groupX;
_textX = format ["Статус %1\n\nЖивих: %2\nБоєздатних: %3\nЗавдання: %4\nБойовий режим:%5\n",groupID _groupX,{alive _x} count _unitsX,{[_x] call A3A_fnc_canFight} count _unitsX,_groupX getVariable ["taskX","Patrol"],behaviour (leader _groupX)];
if ({[_x] call A3A_fnc_isMedic} count _unitsX > 0) then {_textX = format ["%1Є медик\n",_textX]} else {_textX = format ["%1Нема медика\n",_textX]};
if ({_x call A3A_fnc_typeOfSoldier == "ATMan"} count _unitsX > 0) then {_textX = format ["%1Є ПТ\n",_textX]};
if ({_x call A3A_fnc_typeOfSoldier == "AAMan"} count _unitsX > 0) then {_textX = format ["%1Є ППО\n",_textX]};
if (!(isNull(_groupX getVariable ["mortarsX",objNull])) or ({_x call A3A_fnc_typeOfSoldier == "StaticMortar"} count _unitsX > 0)) then
	{
	if ({vehicle _x isKindOf "StaticWeapon"} count _unitsX > 0) then {_textX = format ["%1Міномет розкладено\n",_textX]} else {_textX = format ["%1Міномет не розкладено\n",_textX]};
	}
else
	{
	if ({_x call A3A_fnc_typeOfSoldier == "StaticGunner"} count _unitsX > 0) then
		{
		if ({vehicle _x isKindOf "StaticWeapon"} count _unitsX > 0) then {_textX = format ["%1Станкову зброю розкладено\n",_textX]} else {_textX = format ["%1Станкову зброю не розкладено\n",_textX]};
		};
	};
...
if !(isNull _veh) then
	{
	_textX = format ["%1Техніка:\n%2\n",_textX,getText (configFile >> "CfgVehicles" >> (typeOf _veh) >> "displayName")];
	if (!alive _veh) then
		{
		_textX = format ["%1ЗНИЩЕНО",_textX];
		}
	else
		{
		if (!canMove _veh) then {_textX = format ["%1ВИБИТО\n",_textX]};
		if (count allTurrets [_veh, false] > 0) then
			{
			if (!canFire _veh) then {_textX = format ["%1НЕ МОЖЕ СТРІЛЯТИ\n",_textX]};
			if (someAmmo _veh) then {_textX = format ["%1Є боєзапас\n",_textX]};
			};
		_textX = format ["%1На борту:%2/%3",_textX,{vehicle _x == _veh} count _unitsX,{alive _x} count _unitsX];
		};
	};
