...
_truckX addEventHandler ["GetIn",
	...
	if (_this select 1 == "driver") then
		{
		_textX = format ["Відвезіть цю вантажівку в банк в %1. Припаркуйте її біля парадного входу",(_this select 0) getVariable "destinationX"];
...
[[teamPlayer,civilian],"LOG",[format ["Ми знаємо, що в %1 є банк з купою грошей. Беріть оту вантажівку і під'їдьте до головного входу банку до %2. Стійте там дві хвилини, поки гроші не погрузяться до вантажівки. Коли вони погрузяться, давайте драла до штабу.",_nameDest,_displayTime],"Пограбування банку",_mrkFinal],_positionX,false,0,true,"Interact",true] call BIS_fnc_taskCreate;
...
if ((dateToNumber date > _dateLimitNum) or (!alive _truckX)) then
	{
	["LOG",[format ["Ми знаємо, що в %1 є банк з купою грошей. Беріть оту вантажівку і під'їдьте до головного входу банку до %2. Стійте там дві хвилини, поки гроші не погрузяться до вантажівки. Коли вони погрузяться, давайте драла до штабу.",_nameDest,_displayTime],"Пограбування банку",_mrkFinal],_positionX,"FAILED","Interact"] call A3A_fnc_taskUpdate;
	...
	while {(_countX > 0) or (_truckX distance _positionX < 7) and (alive _truckX) and (dateToNumber date < _dateLimitNum)} do
		...
		if (_countX > 0) then
			{
			_countX = 120*_bonus;//120
			if (_truckX distance _positionX > 6) then {{[petros,"hint","Не відводьте вантажівку задалеко від банку, бо відлік почнеться заново."] remoteExec ["A3A_fnc_commsMP",_x]} forEach ([200,0,_truckX,teamPlayer] call A3A_fnc_distanceUnits)};
			...
			if (alive _truckX) then
				{
				{if (isPlayer _x) then {[petros,"hint","Відвезіть вантажівку до штабу, щоб завершити місію"] remoteExec ["A3A_fnc_commsMP",_x]}} forEach ([80,0,_truckX,teamPlayer] call A3A_fnc_distanceUnits);
				_exit = true;
				};
			//waitUntil {sleep 1; (!alive _truckX) or (_truckX distance _positionX > 7) or (dateToNumber date < _dateLimitNum)};
			};
...
if ((_truckX distance _posbase < 50) and (dateToNumber date < _dateLimitNum)) then
	{
	["LOG",[format ["Ми знаємо, що в %1 є банк з купою грошей. Беріть оту вантажівку і під'їдьте до головного входу банку до %2. Стійте там дві хвилини, поки гроші не погрузяться до вантажівки. Коли вони погрузяться, давайте драла до штабу.",_nameDest,_displayTime],"Пограбування банку",_mrkFinal],_positionX,"SUCCEEDED","Interact"] call A3A_fnc_taskUpdate;
...
if (!alive _truckX) then
	{
	["LOG",[format ["Ми знаємо, що в %1 є банк з купою грошей. Беріть оту вантажівку і під'їдьде до головного входу банку до %2. Стійте там дві хвилини, поки гроші не погрузяться до вантажівки. Коли вони погрузяться, давайте драла до штабу.",_nameDest,_displayTime],"Пограбування банку",_mrkFinal],_positionX,"FAILED","Interact"] call A3A_fnc_taskUpdate;
	[1800*_bonus] remoteExec ["A3A_fnc_timingCA",2];
	[-10*_bonus,theBoss] call A3A_fnc_playerScoreAdd;
	};
