...
[[teamPlayer,civilian],"AS",[format ["Ми довідались про групу %3 спецназу, що патрулює біля %1. Зробіть засідку на тих паскудників до %2, щоб знали де раки зимують. Але будьте обережні, вони трохи посильніші ніж ті шо зазвичай.",_nameDest,_displayTime],"Спецназ",_markerX],_positionX,false,0,true,"Kill",true] call BIS_fnc_taskCreate;
...
if (dateToNumber date > _dateLimitNum) then
	{
	["AS",[format ["Ми довідались про групу %3 спецназу, що патрулює біля %1. Зробіть засідку на тих паскудників до %2, щоб знали де раки зимують. Але будьте обережні, вони трохи посильніші ніж ті шо зазвичай.",_nameDest,_displayTime],"Спецназ",_markerX],_positionX,"FAILED"] call A3A_fnc_taskUpdate;
	if (_difficultX) then
		{
	...
	{
	["AS",[format ["Ми довідались про групу %3 спецназу, що патрулює біля %1. Зробіть засідку на тих паскудників до %2, щоб знали де раки зимують. Але будьте обережні, вони трохи посильніші ніж ті шо зазвичай.",_nameDest,_displayTime,_naming],"Спецназ",_markerX],_positionX,"SUCCEEDED"] call A3A_fnc_taskUpdate;
	if (_difficultX) then
		{
	...
	["TaskFailed", ["", format ["Групу зпецназу в %1 знищено",_nameDest]]] remoteExec ["BIS_fnc_showNotification",_sideX];
	};
